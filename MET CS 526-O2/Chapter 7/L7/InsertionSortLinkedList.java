...
// import NodeLinkedList.Node;

public class InsertionSortLinkedList {

	public static void insertionSort(NodeLinkedList<Integer> list) {
	     NodeLinkedList.Node<Integer> marker = list.first(); // last position known to be sorted
	     while (marker != list.last()) {
	         NodeLinkedList.Node<Integer> pivot = list.after(marker);
	         int value = pivot.getElement();    // number to be placed
	         if (value > marker.getElement())   // pivot is already sorted
	             marker = pivot;
	         else {                             // must relocate pivot
	             NodeLinkedList.Node<Integer> walk = marker;   // find leftmost item greater than value

...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        // Update the product
        $stmt = $pdo->prepare('UPDATE products SET name = ?, description = ?, price = ?, rrp = ?, quantity = ?, img = ?, date_added = ?');
        $stmt->execute([ $_POST['name'], $_POST['description'], $_POST['price'], $_POST['rrp'], $_POST['quantity'], $_POST['main_image'], date('Y-m-d H:i:s', strtotime($_POST['date'])) ]);
    ...
    $page = 'Create';
    if (isset($_POST['submit'])) {
        $stmt = $pdo->prepare('INSERT INTO products (name,description,price,rrp,quantity,img,date_added) VALUES (?,?,?,?,?,?,?,?,?,?)');
        $stmt->execute([ $_POST['name'], $_POST['description'], $_POST['price'], $_POST['rrp'], $_POST['quantity'], $_POST['main_image'], date('Y-m-d H:i:s', strtotime($_POST['date']))]);
        $id = $pdo->lastInsertId();
...
?>
<?=admin_header($page . ' Product', 'products')?>
